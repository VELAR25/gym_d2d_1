...
def test_pl_constant_dB():
    ple = 2.0
    assert pl_constant_dB(2.0, ple) == approx(38.46838313516298)  # pl_constant_dB(The carrier frequencies in Ghz., path loss exponent)
...
class TestLogDistancePathLoss:
    ...
    def test_call(self):
        pl = LogDistancePathLoss(2.1)   # LDPL = 10nlog_10(d) + 10nlog_10(f) + 10nlog_10(4pi/c)
        bs = BaseStation('bs')
        ue = UserEquipment('ue')
